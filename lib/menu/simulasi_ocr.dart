...
import 'package:app_ocgf/tabelgrafik/tabelgrafik_ocr.dart';

class SimulasiOcr extends StatefulWidget {
  const SimulasiOcr({super.key});

  @override
  State<SimulasiOcr> createState() => _SimulasiOcrState();
}

class _SimulasiOcrState extends State<SimulasiOcr>with ChangeNotifier {
  ...
  void _hitungSet() {
    ...
    _arusSettingZona1Controller.text = (1.2 * arusBeban).toString();
  }
  void _providerOcr(){
    //buat provider
    double arusZ1 = double.tryParse(_arusSettingZona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusZ1(arusZ1);
    double tmsZ1 = double.tryParse(_tmsZona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setTmsZ1(tmsZ1);
    double arusM1Z1 = double.tryParse(_arusMoment1Zona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusM1Z1(arusM1Z1);
    double waktuM1Z1 = double.tryParse(_waktuMoment1Zona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setWaktuM1Z1(waktuM1Z1);
    double arusM2Z1 = double.tryParse(_arusMoment2Zona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusM2Z1(arusM2Z1);
    double waktuM2Z1 = double.tryParse(_waktuMoment2Zona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setWaktuM2Z1(waktuM2Z1);

    double arusZ2 = double.tryParse(_arusSettingZona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusZ2(arusZ2);
    double tmsZ2 = double.tryParse(_tmsZona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setTmsZ2(tmsZ2);
    double arusM1Z2 = double.tryParse(_arusMoment1Zona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusM1Z2(arusM1Z2);
    double waktuM1Z2 = double.tryParse(_waktuMoment1Zona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setWaktuM1Z2(waktuM1Z2);
    double arusM2Z2 = double.tryParse(_arusMoment2Zona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setArusM2Z2(arusM2Z2);
    double waktuM2Z2 = double.tryParse(_waktuMoment2Zona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setWaktuM2Z2(waktuM2Z2);

    double panjangZona1 = double.tryParse(_panjangZona1Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setPanjangZona1(panjangZona1);
    double panjangZona2 = double.tryParse(_panjangZona2Controller.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setPanjangZona2(panjangZona2);
    double teganganSekunder = double.tryParse(_teganganSekunderController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setVsekunder(teganganSekunder);
    double impedansiSumber = double.tryParse(_impedansiSumberController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZsumber(impedansiSumber);
    double impedansiTrafo = double.tryParse(_impedansiTrafoController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZtrafo(impedansiTrafo);
    double zG12R = double.tryParse(_zG12RController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZG12r(zG12R);
    double zG12Jx = double.tryParse(_zG12jxController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZG12jx(zG12Jx);
    double zJ12R = double.tryParse(_zJ12RController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZJ12r(zJ12R);
    double zJ12Jx = double.tryParse(_zJ12jxController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setZJ12jx(zJ12Jx);
    double konsA = double.tryParse(_konsAController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setA(konsA);
    double konsB = double.tryParse(_konsBController.text) ?? 0.0;
    Provider.of<DataOcrProvider>(context, listen: false).setB(konsB);
  }
  Future<void> _saveData() async {
    try {
      // Get a reference to the Firestore collection
      CollectionReference hasil = FirebaseFirestore.instance.collection('simulasi-ocr');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Center(
          child: Text('Simulasi OCR', style: TextStyle(color: Colors.white)),
        ),
